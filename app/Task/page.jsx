    ...
    return (
        <div>
            ...
            <div className="mt-5 flex flex-col gap-5">
                <input className="border-2 border-white dark:border-blue-950 rounded-full p-2 w-full outline-none" type="search" placeholder="Search" />
                ...
                <ol className="list-decimal mt-5 flex flex-col gap-2 overflow-y-auto max-h-80">
                    {filteredTask.map((t, index) => (
                        <div key={index} className="">
                            <div className={`flex  flex-col gap-5 text-[0.8rem]  bg-white dark:bg-blue-950 shadow-sm border border-gray-200 dark:border-blue-900 rounded-2xl p-4 hover:shadow-md transition justify-between`}>
                                {edit === t.id ?
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <button onClick={() => handleSave(t.id)} className="bg-green-500 text-white p-1 font-light">Save</button>
                                    </div> :
                                    <li className={`${t.completed ? "line-through " : ""} `}> {t.text}</li>
                                }
                                <hr className="text-blue-900" />
                                <div className="flex gap-2">
                                    <button onClick={() => { setTask(t.text), setEdit(t.id) }} className="bg-green-500 text-white p-1 font-light">Edit</button>
                                    <input checked={t.completed ?? false} onChange={() => toggleComplete(t.id)} type="checkbox" />
                                    <button onClick={() => deleteItem(t.id)} key={index} className="bg-red-500 text-amber-50 p-1 font-light">Del</button>
                                </div>
            ...
            <div className={` mt-10 ${filter === "All" || filter === "Personal" || filter === "Work" || filter === "Wishlist" ? "hidden" : ""}`}>
                ...
                {(!note) && (<p className="text-xs text-red-400 mt-2">Textarea empty</p>)}
                            ...
                            <li  className={`text-[0.8rem]  bg-white dark:bg-blue-950 shadow-sm border border-gray-200 dark:border-blue-900 rounded-2xl p-4 hover:shadow-md transition justify-between`}>
                                {edit === n.id ? 
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <button type="button" onClick={() => handleSaveNote(n.id)} className="bg-green-500 text-white p-1 font-light">Save</button>
                                    </div> :
                                  <span>{n.text}</span>}
                                <div className="flex justify-end  mt-5 gap-3">
                                    <button onClick={() => (setNote(n.text), setEdit(n.id))} className="bg-green-500 text-amber-50 p-1 text-sm font-light">Edit</button>
                                    <button onClick={() => deleteNote(n.id)} className="bg-red-500 text-sm text-amber-50 p-1 font-light">Del</button>
                                </div>
                            </li>
                        </div>
